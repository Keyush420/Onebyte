...
app.get('/bookings/summary', (req, res) => {
    const SQLTotal = 'SELECT COUNT(*) AS total FROM userreservations';
    const SQLPending = 'SELECT COUNT(*) AS pending FROM userreservations WHERE status = "pending"';
    const SQLAccepted = 'SELECT COUNT(*) AS accepted FROM userreservations WHERE status = "accepted"';
    const SQLRejected = 'SELECT COUNT(*) AS rejected FROM userreservations WHERE status = "rejected"';
    ...
    let pendingPromise = new Promise((resolve, reject) => {
        db.query(SQLPending, (err, results) => {
            if (err) reject(err);
            else resolve(results[0].pending);
        });
    });

    let acceptedPromise = new Promise((resolve, reject) => {
        db.query(SQLAccepted, (err, results) => {
            if (err) reject(err);
            else resolve(results[0].accepted);
        });
    });
    ...
    let rejectedPromise = new Promise((resolve, reject) => {
        db.query(SQLRejected, (err, results) => {
            if (err) reject(err);
            else resolve(results[0].rejected);
        });
    });

    Promise.all([totalPromise, pendingPromise, acceptedPromise, rejectedPromise])
